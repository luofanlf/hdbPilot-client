...
class PredActivityforseller : AppCompatActivity() {
        ...
        GlobalScope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@PredActivityforseller, "ONNX model loaded successfully!", Toast.LENGTH_SHORT).show()
                    Log.d("ONNXRT_DEBUG", "ONNX model loaded successfully!")
                }
            } catch (e: Exception) {
                Log.e("ONNXRT", "ONNX model loading failed", e)
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@PredActivityforseller, "ONNX model loading failed: ${e.message}", Toast.LENGTH_LONG).show()
    ...
    private fun performPrediction() {
        ...
        try {
            ...
            Toast.makeText(this@PredActivityforseller, "Prediction complete: $predictedRange", Toast.LENGTH_SHORT).show()
